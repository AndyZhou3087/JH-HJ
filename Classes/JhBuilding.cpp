...
void JhBuilding::parseData(rapidjson::Value& jsonvalue)
{
	if (jsonvalue.IsObject())
	{
		...
		data.level = JhGameDataSave::getInstance()->getBuildLV(data.name);;
...
void JhBuilding::build()
{
	...
	//完成后，等级+1
	data.level++;
	JhGameDataSave::getInstance()->setBuildLV(data.name, data.level);

}

void JhBuilding::action(int minute, int exminute)
{
	//子类自己实现自己的操作
}