...
bool JhRepairOKLayer::init(PackageData* pdata, int type)
{
	m_type = type;
	LayerColor* color = LayerColor::create(Color4B(0, 0, 0, 150));
	this->addChild(color);

	Node* csbnode = CSLoader::createNode("jhrepairOKLayer.csb");
	...
	m_title->setString(JhGlobalData::map_allResource[pdata->strid].cname);
	...
	m_desc->setString(JhCommonFuncs::gbk2utf(strslv.c_str()));
	...
	this->scheduleOnce(schedule_selector(JhRepairOKLayer::showText), 0.6f);

	return true;
}

JhRepairOKLayer* JhRepairOKLayer::create(PackageData* pdata, int type)
{
	JhRepairOKLayer *pRet = new JhRepairOKLayer();
...
void JhRepairOKLayer::showText(float dt)
{
	...
	this->scheduleOnce(schedule_selector(JhRepairOKLayer::removeself), 3.5f);
}

void JhRepairOKLayer::removeself(float dt)
{
