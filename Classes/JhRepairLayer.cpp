...
JhRepairLayer* JhRepairLayer::create(int type)
{
	JhRepairLayer *pRet = new JhRepairLayer();
...
bool JhRepairLayer::init(int type)
{
	m_type = type;
	Node* csbnode = CSLoader::createNode("jhrepairLayer.csb");
	this->addChild(csbnode);

	cocos2d::ui::Widget *backbtn = (cocos2d::ui::Widget*)csbnode->getChildByName("backbtn");
	backbtn->addTouchEventListener(CC_CALLBACK_2(JhRepairLayer::onBack, this));
	...
	m_actionBtn->addTouchEventListener(CC_CALLBACK_2(JhRepairLayer::onAction, this));
	...
	if (m_type == 2)
	{
		m_actionBtn->setTitleText(JhCommonFuncs::gbk2utf("强化"));
	...
	repairResImg->addTouchEventListener(CC_CALLBACK_2(JhRepairLayer::onResItem, this));
	...
	for (int i = 0; i < 3; i++)
	{
		...
		needResImg[i]->addTouchEventListener(CC_CALLBACK_2(JhRepairLayer::onResItem, this));
...
void JhRepairLayer::onEnterTransitionDidFinish()
{
...
void JhRepairLayer::onBack(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void JhRepairLayer::onAction(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	JhCommonFuncs::BtnAction(pSender, type);
	if (type == ui::Widget::TouchEventType::ENDED)
	{
		...
		if (node->getTag() == 1)
		{
			...
			{
				JhHintBox* layer = JhHintBox::create(JhCommonFuncs::gbk2utf("无需修复！！"));
				this->addChild(layer);
				return;
			}
			else if (!resIsEnough)
			{
				JhHintBox* layer = JhHintBox::create(JhCommonFuncs::gbk2utf("资源不足！！"));
				this->addChild(layer);
				return;
			}
			else
		...
		else
		{
			if (data->slv >= 20)
			{
				JhHintBox* layer = JhHintBox::create(JhCommonFuncs::gbk2utf("强化已最高，无需强化！！"));
				this->addChild(layer);
				return;
			}
			else if (!resIsEnough)
...
void JhRepairLayer::onRepairItem(cocos2d::Ref* pSender)
{
	...
	if (pdata->strid.compare("82") == 0)
	{
		if (clickindex < 0)
		{
			JhHintBox* hint = JhHintBox::create(JhCommonFuncs::gbk2utf("请先选择装备！！"));
			this->addChild(hint);
			return;
		}
		...
		succrandtext->setString(JhCommonFuncs::gbk2utf(strrnd.c_str()));
	...
	if (m_type == 1)
	{
		repairneedres = JhGlobalData::map_equips[pdata->strid].repair_res;
	}
	else
	{
		repairneedres.clear();
...
void JhRepairLayer::onResItem(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	if (type == ui::Widget::TouchEventType::ENDED)
	{
		Node* node = (Node*)pSender;
		...
		if (node->getTag() == 0)
		{
			...
			if (pdata != NULL && pdata->strid.length() > 0)
				this->addChild(JhResDetailsLayer::create((PackageData*)node->getUserData()));
		}
		else
		{
			std::string resid = StringUtils::format("%d", node->getTag());
			this->addChild(JhResDetailsLayer::createByResId(resid));
		}
			
	}
}

void JhRepairLayer::showAnim()
{
	m_actionBtn->setEnabled(false);
	...
	repairbox->runAction(Sequence::create(EaseSineIn::create(RotateTo::create(1.5f, 720 * 2)), CallFuncN::create(CC_CALLBACK_1(JhRepairLayer::finishAnim, this)), NULL));
}

int JhRepairLayer::getMyPackageResCount(std::string strid)
{
	int count = 0;
...
void JhRepairLayer::finishAnim(Ref* pSender)
{
	float delay = 0.05f;
	...
	if (m_type == 2)
	{
		...
		if (r < succrnd)
		{
			float dt = 0.5f;
			data->slv += 1;
			this->scheduleOnce(schedule_selector(JhRepairLayer::delayShowOkLayer), dt);
			delay += dt;

			for (unsigned int i = 0; i < JhGlobalData::vec_achiveData.size(); i++)
			{
				if (JhGlobalData::vec_achiveData[i].type == A_9)
				{
					if (JhGlobalData::vec_achiveData[i].vec_para[0].compare(data->strid) == 0 && JhGlobalData::vec_achiveData[i].finish != -1)
					{
						JhGlobalData::vec_achiveData[i].finish = data->slv;
	...
	if (useluckstone > 0)
	{
		JhMyPackage::cutone("82", useluckstone);
		useluckstone = 0;
	}
	for (unsigned int i = 0; i < repairneedres.size(); i++)
	{
		...
		JhMyPackage::cutone(strid, needrescount);
	}

	updataUI();
	this->scheduleOnce(schedule_selector(JhRepairLayer::delayCanClick), delay);
}

void JhRepairLayer::updataUI()
{
	...
	{
		if (JhMyPackage::vec_packages[i].type == WEAPON || JhMyPackage::vec_packages[i].type == PROTECT_EQU)
			vec_RepairData.push_back(&JhMyPackage::vec_packages[i]);
		if (m_type == 2 && JhMyPackage::vec_packages[i].type == RES_2 && JhMyPackage::vec_packages[i].strid.compare("82") == 0)
		{
			vec_RepairData.push_back(&JhMyPackage::vec_packages[i]);
		}
	}
	...
	for (unsigned int i = 0; i < vec_RepairData.size(); i++)
	{
		...
		MenuItemSprite* boxItem = MenuItemSprite::create(
			...
			box,
			box,
			CC_CALLBACK_1(JhRepairLayer::onRepairItem, this));
...
void JhRepairLayer::delayShowOkLayer(float dt)
{
	JhRepairOKLayer* layer = JhRepairOKLayer::create((PackageData*)repairResImg->getUserData(), m_type);
	this->addChild(layer);
...
void JhRepairLayer::delayCanClick(float dt)
{
	g_gameLayer->removeChildByName("notouchlayer");
}

void JhRepairLayer::dofail()
{
	...
	else
	{
		...
		for (int i = 0; i < JhMyPackage::getSize(); i++)
		{
			std::string strid = JhMyPackage::vec_packages[i].strid;
			if (strid.compare(vec_RepairData[clickindex]->strid) == 0)
			{
				JhMyPackage::cutone(JhMyPackage::vec_packages[i]);
			}
		}
	}
}

void JhRepairLayer::onExit()
{
	Layer::onExit();
}