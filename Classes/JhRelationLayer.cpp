...
JhRelationLayer* JhRelationLayer::create()
{
	JhRelationLayer *pRet = new JhRelationLayer();
	if (pRet && pRet->init())
	{
		pRet->autorelease();
	}
...
bool JhRelationLayer::init()
{
	...
	backbtn->addTouchEventListener(CC_CALLBACK_2(JhRelationLayer::onBack, this));
...
void JhRelationLayer::onEnterTransitionDidFinish()
{
	Layer::onEnterTransitionDidFinish();
	this->scheduleOnce(schedule_selector(JhRelationLayer::delayShowData), 0.1f);
}

void JhRelationLayer::onBack(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void JhRelationLayer::delayShowData(float dt)
{
	int size = JhGlobalData::map_NPCFriendData.size();
	...
	for (it = JhGlobalData::map_NPCFriendData.begin(); it != JhGlobalData::map_NPCFriendData.end(); ++it)
	{
		std::string npcid = it->first;

		Node* nodeitem = CSLoader::createNode("jhrelationNode.csb");
		...
		std::string propestr = StringUtils::format("ui/npcprope%d.png", JhGlobalData::map_npcs[npcid].type);
		...
		relation->setString(JhCommonFuncs::gbk2utf(relationdesc[JhGlobalData::map_myfriendly[npcid].relation].c_str()));


		int friendly = JhGlobalData::map_myfriendly[npcid].friendly;

		if (friendly < 0)
			relation->setString(JhCommonFuncs::gbk2utf("仇人"));
