...
int JhCommonFuncs::split(const std::string& str, std::vector<std::string>& ret_, std::string sep)
{
	if (str.empty())
	{
		return 0;
...
std::string JhCommonFuncs::replace_all(std::string & s, std::string const & t, std::string const & w)
{
...
int JhCommonFuncs::code_convert(const char *from_charset, const char *to_charset, const char *inbuf, size_t inlen, char *outbuf, size_t outlen)
{
...
std::string JhCommonFuncs::gbk2utf(const char *inbuf)
{
	std::string strRet;
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)
...
void JhCommonFuncs::BtnAction(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void JhCommonFuncs::removeGray(cocos2d::Node* node)
{
