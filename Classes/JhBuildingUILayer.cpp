...
JhBuildingUILayer* JhBuildingUILayer::create(JhBuilding* build)
{
	JhBuildingUILayer *pRet = new JhBuildingUILayer();
...
bool JhBuildingUILayer::init(JhBuilding* build)
{
	m_build = build;
	...
	m_backbtn->addTouchEventListener(CC_CALLBACK_2(JhBuildingUILayer::onBack, this));
	...
	buildbtn->addTouchEventListener(CC_CALLBACK_2(JhBuildingUILayer::onAction, this));
	...
	buildicon->addTouchEventListener(CC_CALLBACK_2(JhBuildingUILayer::onBuidingDetails, this));
	...
	needtimelbl->setString(JhCommonFuncs::gbk2utf(needtimestr.c_str()));
	...
	if (strcmp(m_build->data.name, "forgingtable") == 0)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			btn->setTag(i);
			btn->addTouchEventListener(CC_CALLBACK_2(JhBuildingUILayer::onCategory, this));
	...
	this->scheduleOnce(schedule_selector(JhBuildingUILayer::delayShowNewerGuide), 0.2f);
	return true;
}

void JhBuildingUILayer::onEnterTransitionDidFinish()
{
	Layer::onEnterTransitionDidFinish();
}

void JhBuildingUILayer::onBack(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	...
	if (type == ui::Widget::TouchEventType::ENDED)
	{
		...
		else if (JhNewerGuideLayer::checkifNewerGuide(45))
		{
			JhHomeLayer* homlayer = (JhHomeLayer*)g_gameLayer->getChildByName("homelayer");
			if (homlayer != NULL)
...
void JhBuildingUILayer::setActionScrollViewUI()
{
	...
	this->scheduleOnce(schedule_selector(JhBuildingUILayer::delayLoadActionUi), 0.1f);
...
void JhBuildingUILayer::loadActionUi()
{
	int itemheight = 160;
	...
	for (unsigned int i = 0; i < vec_actionItem.size(); i++)
	{
		...
		/*建筑物操作的显示ICON*/
		icon->addTouchEventListener(CC_CALLBACK_2(JhBuildingUILayer::onResDetails, this));
		icon->setTag(10000 * (i + 1));//点击按钮TAG来区分10000以上 
		icon->setSwallowTouches(false);

		cocos2d::ui::Button* actbtn = (cocos2d::ui::Button*)item->getChildByName("actionbtn");
		actbtn->addTouchEventListener(CC_CALLBACK_2(JhBuildingUILayer::onAction, this));
		...
		if (type == WEAPON || type == PROTECT_EQU)
		{
			if (g_hero->checkifHasGF_Equip(vec_buildAcitonData.at(i).icon))
				actbtn->setTitleText(JhCommonFuncs::gbk2utf("分解"));
			else if (JhGlobalData::tempHasGf_Equip(vec_buildAcitonData.at(i).icon).length() > 0)
			{
				actbtn->setTitleText(JhCommonFuncs::gbk2utf("已有"));
			}
		}
...
void JhBuildingUILayer::delayLoadActionUi(float dt)
{
...
void JhBuildingUILayer::onAction(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	JhCommonFuncs::BtnAction(pSender, type);
	if (type == ui::Widget::TouchEventType::ENDED)
	{
		...
		int tag = node->getTag();
		if (JhGlobalData::isExercising() && !JhGlobalData::isHasFSF() && strcmp(m_build->data.name, "exerciseroom") != 0)
		{
			int index = -1;
		...
		if (tag == BUILD)//建造或者升级
		{
			...
			buildbar->runAction(Sequence::create(MyProgressTo::create(ACTION_BAR_TIME, 100), CallFuncN::create(CC_CALLBACK_1(JhBuildingUILayer::onfinish, this, BUILD)), NULL));
			m_build->build();
		}
		else//操作
		{
			...
			if (node->getTitleText().compare(JhCommonFuncs::gbk2utf("已有")) == 0)
			{
				std::string mapid = JhGlobalData::tempHasGf_Equip(vec_buildAcitonData.at(tag - ACTION).icon);
				std::string hasstr = StringUtils::format("%s%s%s", JhGlobalData::map_maps[mapid].cname, JhCommonFuncs::gbk2utf("临时存放点存有").c_str(), vec_buildAcitonData.at(tag - ACTION).cname.c_str());
				JhHintBox* layer = JhHintBox::create(hasstr);
				this->addChild(layer);
				return;
			}

			if (node->getTitleText().compare(JhCommonFuncs::gbk2utf("分解")) == 0)
			{
				JhDivideLayer* layer = JhDivideLayer::create(&vec_buildAcitonData.at(tag - ACTION));
				g_gameLayer->addChild(layer, 4);
				vec_actionbar[tag - ACTION]->runAction(Sequence::create(MyProgressTo::create(m_build->getActionBarTime(), 100), CallFuncN::create(CC_CALLBACK_1(JhBuildingUILayer::ondivideSucc, this, (BACTIONTYPE)tag, layer)), NULL));
			...
			{
				JhHintBox* hintbox = JhHintBox::create(JhCommonFuncs::gbk2utf("暖炉中正在生火！请稍后再操作"));
			...
			if (strcmp(m_build->data.name, "exerciseroom") == 0)
			{
				...
				{
					JhExerciseCancelLayer* layer = JhExerciseCancelLayer::create();
			...
			{
				m_backbtn->setEnabled(false);
				vec_actionbar[tag - ACTION]->runAction(Sequence::create(MyProgressTo::create(m_build->getActionBarTime(), 100), CallFuncN::create(CC_CALLBACK_1(JhBuildingUILayer::onfinish, this, (BACTIONTYPE)tag)), NULL));
			}
			m_build->action(actime, extime);
		}
	}
...
void JhBuildingUILayer::ondivideSucc(Ref* pSender, BACTIONTYPE type, Node* divideLayer)
{
	g_nature->setTimeInterval(NORMAL_TIMEINTERVAL);
	vec_actionbar[type - ACTION]->setPercent(0);
	vec_actionbtn[type - ACTION]->setTitleText(JhCommonFuncs::gbk2utf("锻造"));
	JhDivideLayer* layer = (JhDivideLayer*)divideLayer;
	layer->showContent();
...
void JhBuildingUILayer::onfinish(Ref* pSender, BACTIONTYPE type)
{
	g_nature->setTimeInterval(NORMAL_TIMEINTERVAL);
	if (type == BUILD)//建造或升级完成
	{
		...
		{
			Director::getInstance()->getRunningScene()->addChild(JhBookShelfLayer::create(), 4);
			this->removeFromParentAndCleanup(true);
		}
	}
	else//操作完成
	{
		...
		{
			vec_actionbtn[type - ACTION]->setTitleText(JhCommonFuncs::gbk2utf("分解"));
		}
...
void JhBuildingUILayer::updataBuildRes()
{
	...
	if (level > 0)
	{
		...
		if (level == m_build->data.maxlevel)
		{
			...
			buildbtn->setEnabled(false);
			buildbtn->setTitleText(JhCommonFuncs::gbk2utf("最高级"));
			buildnode->getChildByName("item")->getChildByName("needtime")->setVisible(false);
		}
		else
		{
			buildbtn->setTitleText(JhCommonFuncs::gbk2utf("升级"));
		}
	}
...
void JhBuildingUILayer::updataActionRes()
{
	//更新操作需要的资源
	...
	for (unsigned int i = 0; i < vec_actionItem.size(); i++)
	{
		...
		else
		{
			...
			desc->setString(JhCommonFuncs::gbk2utf("需要更高等级的建筑！"));
...
void JhBuildingUILayer::onResDetails(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	...
	else if (type == ui::Widget::TouchEventType::ENDED)
	{
		if (!isDraging)
		{
			...
			JhResDetailsLayer::whereClick = 1;
			this->addChild(JhResDetailsLayer::createByResId(strid));
		}
	}
}

void JhBuildingUILayer::onBuidingDetails(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	if (type == ui::Widget::TouchEventType::ENDED)
	{
		JhSoundManager::getInstance()->playSound(JhSoundManager::SOUND_ID_BUTTON);
...
void JhBuildingUILayer::showFinishHintText(std::string path)
{
	Sprite* bsprite = Sprite::create(path);
	bsprite->setPosition(Vec2(360, 600));
	this->addChild(bsprite);
	bsprite->runAction(Sequence::create(FadeOut::create(1.5f), CallFuncN::create(CC_CALLBACK_1(JhBuildingUILayer::finishAnim, this, bsprite)), NULL));
}

void JhBuildingUILayer::finishAnim(Ref* pSender, Node* node)
{
	node->removeFromParentAndCleanup(true);
}

void JhBuildingUILayer::showNewerGuide(int step)
{
...
void JhBuildingUILayer::delayShowNewerGuide(float dt)
{
	if (strcmp(m_build->data.name, "bed") == 0)
	{
		if (JhNewerGuideLayer::checkifNewerGuide(1) && m_build->data.level <= 0)
			showNewerGuide(1);
		else if (JhNewerGuideLayer::checkifNewerGuide(44))
			showNewerGuide(44);
	}
}

void JhBuildingUILayer::onExercisefinish(int index)
{
	vec_actionbtn[index]->setTitleText(JhCommonFuncs::gbk2utf("出关"));
	this->unschedule(schedule_selector(JhBuildingUILayer::updateExerciseLeftTime));
	vec_progresstext[index]->setVisible(false);
}

void JhBuildingUILayer::updateExerciseDesc()
{
	cocos2d::ui::Text* importdesc = (cocos2d::ui::Text*)buildnode->getChildByName("item")->getChildByName("desc");
	importdesc->setVisible(true);
	importdesc->setString(JhCommonFuncs::gbk2utf(exersiceDesc.c_str()));
}

void JhBuildingUILayer::updateExerciseLeftTime(float dt)
{
...
void JhBuildingUILayer::resetExercise()
{
	this->unschedule(schedule_selector(JhBuildingUILayer::updateExerciseLeftTime));
	JhGameDataSave::getInstance()->setExersiceCfg("");
	...
	vec_actionbtn[selectActionIndex]->setTitleText(JhCommonFuncs::gbk2utf("闭关"));
	for (unsigned int i = 0; i < vec_actionbtn.size(); i++)
	{
		vec_actionbtn[i]->setEnabled(true);
	}
...
void JhBuildingUILayer::onCategory(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	JhCommonFuncs::BtnAction(pSender, type);
	if (type == ui::Widget::TouchEventType::ENDED)
	{
		...
		this->scheduleOnce(schedule_selector(JhBuildingUILayer::delayShowCategoryActionUI), 0.1f);
	}
}

void JhBuildingUILayer::loadActionUIByCategory(int category)
{
	vec_buildAcitonData.clear();
	int size = JhGlobalData::map_buidACData["forgingtable"].size();
	if (category == 0)
	{
		for (int i = 0; i < size; i++)
			vec_buildAcitonData.push_back(JhGlobalData::map_buidACData["forgingtable"][i]);
	}
	else if (category == 1)
	{
		for (int i = 0; i < size; i++)
		{
			int stype = JhGlobalData::map_buidACData["forgingtable"][i].type - 1;
			if (stype == RES_1)
				vec_buildAcitonData.push_back(JhGlobalData::map_buidACData["forgingtable"][i]);
		}
	}
...
void JhBuildingUILayer::delayShowCategoryActionUI(float dt)
{
	loadActionUIByCategory(lastCategoryindex);
}

void JhBuildingUILayer::getServerTime()
{
	if (strcmp(m_build->data.name, "exerciseroom") == 0)
	{
...
void JhBuildingUILayer::onSuccess()
{
	Director::getInstance()->getRunningScene()->removeChildByName("waitbox");
	if (exersiceTag > 0)
	{
		if (vec_actionbtn[exersiceTag - ACTION]->getTitleText().compare(JhCommonFuncs::gbk2utf("闭关")) == 0)
		{
			estarttime = JhGlobalData::servertime;
			vec_actionbtn[exersiceTag - ACTION]->setTitleText(JhCommonFuncs::gbk2utf("取消"));
			curtime = JhGlobalData::servertime;
			updateExerciseLeftTime(0);
			this->schedule(schedule_selector(JhBuildingUILayer::updateExerciseLeftTime), 1);
			...
			JhGameDataSave::getInstance()->setExersiceCfg(estr);
		}
		else if (vec_actionbtn[exersiceTag - ACTION]->getTitleText().compare(JhCommonFuncs::gbk2utf("出关")) == 0)
		{
			int extime = vec_buildAcitonData.at(exersiceTag - ACTION).extime;
			if (JhGlobalData::servertime + 2 - estarttime >= extime * 60)
			{
				JhExerciseDoneLayer* layer = JhExerciseDoneLayer::create(ex_wgstrid, ex_ngstrid, extime / 60);
				this->addChild(layer, 4);
				resetExercise();
			}
			else
			{
				JhHintBox* hbox = JhHintBox::create(JhCommonFuncs::gbk2utf("出关失败"));
	...
	else
	{
		if (strcmp(m_build->data.name, "exerciseroom") == 0)
		{
			std::string estr = JhGameDataSave::getInstance()->getExersiceCfg();
			int index = -1;

			if (estr.length() > 0)
			{
				std::vector<std::string> tmp;
				JhCommonFuncs::split(estr, tmp, "-");
				if (tmp.size() >= 4)
				{
			...
			if (index >= 0)
			{
				...
				{
					vec_actionbtn[index]->setTitleText(JhCommonFuncs::gbk2utf("取消"));
					updateExerciseLeftTime(0);
					this->schedule(schedule_selector(JhBuildingUILayer::updateExerciseLeftTime), 1);
				}
...
void JhBuildingUILayer::onErr(int errcode)
{
	Director::getInstance()->getRunningScene()->removeChildByName("waitbox");
	JhHintBox* hbox = JhHintBox::create(JhCommonFuncs::gbk2utf("失败!!!"));
	this->addChild(hbox, 4);
}