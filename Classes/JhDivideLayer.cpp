...
void JhDivideLayer::showContent()
{
	...
	for (int i = 0; i < datasize; i++)
	{
		...
		Label * namelbl = Label::createWithTTF(JhGlobalData::map_allResource[strid].cname, "fonts/STXINGKA.TTF", 25);
	...
	this->scheduleOnce(schedule_selector(JhDivideLayer::removeSelf), 3.0f);
}

void JhDivideLayer::removeSelf(float dt)
{
	this->removeFromParentAndCleanup(true);
}

JhDivideLayer* JhDivideLayer::create(BuildActionData* bdata)
{
	JhDivideLayer *pRet = new JhDivideLayer();
