...
Scene* JhActivitScene::createScene(std::string imagepath, std::string content)
{
	...
	auto layer = JhActivitScene::create(imagepath, content);
...
bool JhActivitScene::init(std::string imagepath, std::string content)
{
	...
	tips->setString(JhCommonFuncs::gbk2utf(str.c_str()));

	float distime = 3.5f;
	if (imagepath.compare("images/cday.jpg") == 0 && content.compare(JhCommonFuncs::gbk2utf("今夜很平静，新的一天开始...")) == 0)
	{
	...
	{
		tips->setString(JhCommonFuncs::gbk2utf("后山的三七、降真香、野菜、水果数量减半！"));
		distime = 3.0f;
	}
	else if (imagepath.compare("images/hunter.jpg") == 0)
	{
		tips->setString(JhCommonFuncs::gbk2utf("最近上山打猎的人增多，兔子和狼都快被打没了！"));
		distime = 3.0f;
	}
	else if (imagepath.compare("images/thieves.jpg") == 0)
	{
		...
		{
			this->scheduleOnce(schedule_selector(JhActivitScene::checkstoleData), 0.1f);
			distime = 3.5f;
			tips->setString(JhCommonFuncs::gbk2utf("tip：可在商城购买【看门雕】预防盗贼！"));
		...
		else
		{
			...
			m_text->setString(JhCommonFuncs::gbk2utf("盗贼前来偷窃，被看门雕及时发现，避免了不必要的损失！"));
			distime = 3.5f;
		}
	...
	{
		int rj = JhGlobalData::createRandomNum(100);
		if (rj < 50)
		{
			image->loadTexture("images/jumphurt.jpg", cocos2d::ui::TextureResType::LOCAL);
			m_text->setString(JhCommonFuncs::gbk2utf("跳崖摔伤"));
			...
			tips->setString(JhCommonFuncs::gbk2utf("没想到这个悬崖这么高！！！受伤严重！！！"));
			distime = 3.0f;
#ifdef ANALYTICS
			JhAnalyticUtil::onEvent("jumphurt");
#endif
		}
		else
		{
			int rf = JhGlobalData::createRandomNum(100);
			if (rf < 50)
			{
				image->loadTexture("images/jumpnothing.jpg", cocos2d::ui::TextureResType::LOCAL);
				m_text->setString(JhCommonFuncs::gbk2utf("一无所获"));
				tips->setString(JhCommonFuncs::gbk2utf("这次白跳了，什么都没有，发光的只不过时一个破石头在反光。"));
				distime = 3.0f;
#ifdef ANALYTICS
				JhAnalyticUtil::onEvent("jumpnoting");
#endif
			}
			else
			{
				this->scheduleOnce(schedule_selector(JhActivitScene::getRndRes), 0.2f);
				//text->setString(JhCommonFuncs::gbk2utf("机缘巧合"));
				tips->setString(JhCommonFuncs::gbk2utf("果然没有白白冒险，没想到这悬崖下别有洞天！你获得了以下物品："));
				tips->setPositionY(m_text->getPositionY() + 20);

#ifdef ANALYTICS
				JhAnalyticUtil::onEvent("jumpget");
#endif
	...
	this->scheduleOnce(schedule_selector(JhActivitScene::popself), distime);
	return true;
}

void JhActivitScene::checkstoleData(float dt)
{
	...
	if (!istemphas)
	{
		...
		for (it = StorageRoom::map_storageData.begin(); it != StorageRoom::map_storageData.end(); ++it)
		{
			...
			for (int j = 0; j < size; j++)
			{
				...
				{
					bool isadd = true;
					if ((sdata.type == WEAPON || sdata.type == PROTECT_EQU) && JhGlobalData::map_equips[sdata.strid].qu >= 5)
					{
						isadd = false;
					}
					else if ((sdata.type == N_GONG || sdata.type == W_GONG) && JhGlobalData::map_wgngs[sdata.strid].qu >= 5)
					{
						isadd = false;
					}
	...
	else
	{
		...
		int r = JhGlobalData::createRandomNum(tempmapsize);
	...
	if (m_stoleData.size() > 0)
	{
		...
		int r = JhGlobalData::createRandomNum(rcount) + 1;
		int startx[] = { 360, 300, 240, 180, 120 };
		int spacex[] = { 120, 120, 120, 120, 120 };
		srand(JhGlobalData::getSysSecTime());
		...
		for (int i = 0; i < r; i++)
		{
			...
			Label * namelbl = Label::createWithTTF(JhGlobalData::map_allResource[tmpdata.strid].cname, "fonts/STXINGKA.TTF", 25);
			...
			int r1 = JhGlobalData::createRandomNum(rndcount) + 1;
		...
		{
			saveTempResData();
			std::string desc = StringUtils::format("%s%s%s", JhCommonFuncs::gbk2utf("放在").c_str(), JhGlobalData::map_maps[m_tempmapid].cname, JhCommonFuncs::gbk2utf("的物品被盗贼偷走了！").c_str());
			m_text->setString(desc);
		}
	}
	else
	{
		m_text->setString(JhCommonFuncs::gbk2utf("盗贼到来，没有找到任何可偷的物品..."));
	}
}

void JhActivitScene::getRndRes(float dt)
{
	...
	if (r < 40)
	{
		...
		{
			if (JhGlobalData::tempHasGf_Equip(vec_tempgf[i]).length() <= 0)
			{
		...
		{
			r = JhGlobalData::createRandomNum(vec_randRes.size());
			PackageData data;
			data.strid = vec_randRes[r];
			data.type = JhGlobalData::map_wgngs[data.strid].type - 1;
			data.extype = JhGlobalData::map_wgngs[data.strid].extype;
			data.count = 1;
	...
	if (r1 < 40)
	{
		...
		{
			if (JhGlobalData::tempHasGf_Equip(vec_tempequip[i]).length() <= 0)
				vec_randRes.push_back(vec_tempequip[i]);
		}
		if (vec_randRes.size() > 0)
		{
			r1 = JhGlobalData::createRandomNum(vec_randRes.size());

			PackageData data;
			data.strid = vec_randRes[r1];
			data.type = JhGlobalData::map_equips[data.strid].type - 1;
			data.extype = JhGlobalData::map_equips[data.strid].extype;
			data.count = 1;
			vec_randData.push_back(data);
		}
	}
	...
	int r3 = JhGlobalData::createRandomNum(3) + 1;
	for (int i = 0; i < r3; i++)
	{
		PackageData data;
		data.strid = vec_randRes[i];
		data.type = JhGlobalData::getResType(data.strid);
		data.count = 10;
		vec_randData.push_back(data);
	}
...
JhActivitScene* JhActivitScene::create(std::string imagepath, std::string content)
{
	JhActivitScene *pRet = new JhActivitScene();
...
void JhActivitScene::popself(float dt)
{
	Director::getInstance()->popScene();
}

void JhActivitScene::saveTempResData()
{
	JhGlobalData::map_tempGf_Equip[m_tempmapid].clear();
	std::string str;
	...
	for (int i = 0; i < size; i++)
	{
		...
		{
			JhGlobalData::map_tempGf_Equip[m_tempmapid].push_back(tmpstrid);
		}
	}
	JhGameDataSave::getInstance()->setTempStorage(m_tempmapid, str.substr(0, str.length() - 1));
}