...
JhHttpUtil* JhHttpUtil::getInstance()
{
	JhHttpUtil* httputil = new JhHttpUtil();
	httputil->autorelease();
	httputil->retain();
	return httputil;
}

void JhHttpUtil::doData(std::string url, HttpCBFunc http_cb, std::string filename, HTTPTYPE method, std::string postdata, std::string tag)
{
	http_cbfunc = http_cb;
	m_filename = filename;
	m_tag = tag;
	...
	httpReq->setResponseCallback(CC_CALLBACK_2(JhHttpUtil::onHttpRequestCompleted, this));
...
void JhHttpUtil::onHttpRequestCompleted(cocos2d::network::HttpClient *sender, cocos2d::network::HttpResponse *resp)
{
	std::string retdata;
	int retcode = 0;
